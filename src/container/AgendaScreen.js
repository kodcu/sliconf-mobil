...
class AgendaScreen extends Component {
	...
	componentWillReceiveProps(nextProps) {
		if (nextProps.schedule && nextProps.schedule !== this.props.schedule) {
			if (nextProps.schedule.length > -1 && Boolean(nextProps.schedule)) {
	...
	isThereEventInRoom(myroom, talkListByTime) {
		...
		for (let i = 0; i < talkListByTime.length; i++) {
			if (myroom === talkListByTime[i].room)
				return this.getAgendaCard(
					talkListByTime,
					i,
					this.isChoosen(chosen, talkListByTime[i])
				);
		}
		return (<AgendaCard isEmpty />);
	}
	...
	render() {
		...
		const talksList = this.state.data;
		...
		return (
			<View style={styles.container}>
				<If con={isChosenClicked}>
					<If.Then>
						<FilterEvent
							visible={filter} onPress={(e) => this.filterHide(e)}
						...
						>
							<Picker
								...
								{eventsData.map((item, i) =>
									<Picker.Item
										key={i + 1} label={moment(item, "MM-DD-YYYY").format("dddd")}
										value={item}
									/>
								)}
							</Picker>
														...
														<If.Then>
															<View style={styles.cardsField}>
																{rooms.map((myroom, i) => (
																	<View
																		key={i}
																	>{this.isThereEventInRoom(myroom, talksList[time])}</View>
																))}
															</View>
														</If.Then>
														<If.Else>
